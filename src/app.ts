...
app.post('/medicos', async (req, res) => {
  try {
    ...
    const hash = await registerUser(datosMedico.PasswordMedico);
...
function registerUser(password: string): Promise<string> {
  const saltRounds = 10;
...
app.get('/recetasmedicamento', async (req, res) => {
  const db = new Database();
...
app.post('/api/login', async (req, res) => {
  ...
  try {
    ...
    const validPassword = await bcrypt.compare(password, user.PasswordMedico);
...
async function getUserByEmail(email: string) {
  ...
  try {
    const results = await db.query('SELECT * FROM medicos WHERE CorreoMedico = ?', [email]);
    if (results.length === 0) {
      return null;
    }
    console.log(results);
...
app.get('/citasPorMes2024', async (req: Request, res: Response) => {
  const db = new Database();
...
`;
      ...
      db.query(consultaSQL, [inicio2024, fin2024])
        .then((results: any[]) => {
          const citasPorMes = results.map((cita: any) => ({
            mes: cita.mes,
            cantidadCitas: cita.cantidadCitas
          }));
